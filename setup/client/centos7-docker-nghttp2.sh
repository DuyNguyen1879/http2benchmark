...
h2load_install() {
  ...
  if [[ "$INSTALL_NGHTTPNEW" = [yY] ]]; then
    ...
    # fi
    echo "if [[ \"$(docker inspect nghttp-min | jq -r '.[] | .State.Status')\" != 'running' ]]; then docker restart nghttp-min >/dev/null 2>&1; sleep 1; fi; docker exec -ti nghttp-min nghttp \"\$@\"" > /usr/bin/nghttpnew
  ...
  if [[ "$INSTALL_NGHTTPNEWER" = [yY] ]]; then
    ...
    # fi
    echo "if [[ \"$(docker inspect nghttp | jq -r '.[] | .State.Status')\" != 'running' ]]; then docker restart nghttp >/dev/null 2>&1; sleep 1; fi; docker exec -ti nghttp nghttp \"\$@\"" > /usr/bin/nghttpnewer
