...
### Install 1kb static file precompressed
    if [ ! -e ${DOCROOT}/1kgzip-static.html ]; then
...
### Install 10kb static file precompressed
    if [ ! -e ${DOCROOT}/10kgzip-static.html ]; then
...
### Install 100kb static file
    if [ ! -e ${DOCROOT}/100kstatic.html ]; then
        ...
        echoG '100kstatic.html already exist'   
    fi
...
cpuprocess(){
    if [[ ${CPUNUM} > 1 ]]; then
        ...
        ### OLS
        if [ -f "${OLSDIR}/conf/httpd_config.conf" ]; then
